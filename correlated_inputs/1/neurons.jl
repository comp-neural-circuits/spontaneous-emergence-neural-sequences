...
@with_kw mutable struct NeuronArray
    ...
    groups::Array{Int}{2} = reshape(1:(M*group_size), group_size, M)
...
function update_neurons!(G::NeuronArray, dt::Float, t::Float)
    ...
    if G.M > 0
        ...
        corr_period = Int(ceil(t/(G.param.corr_t + G.param.gap_t)))
        corr_idx = mod((corr_period-1), G.M)+1
        if t - (corr_period-1)*(G.param.corr_t + G.param.gap_t) <= G.param.corr_t
