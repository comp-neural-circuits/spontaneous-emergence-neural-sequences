        ...
        for trial = 1:trials
                ...
                for i = 1:length(all_ckpt)
                        ...
                        for corr_seg = 1:round(EE_seg.m/corr_range)
                                if corr_seg < round(EE_seg.m/corr_range)
                                        index_ee_strong{i} = [index_ee_strong{i};
                                                find(effw_EE > thresh_EE & EE_seg.rowval >= corr_seg*corr_range + 1 & ...
                                                EE_seg.rowval <= (corr_seg+1)*corr_range & EE_colval >= (corr_seg - 1)*corr_range + 1 & ...
                                                EE_colval <= corr_seg*corr_range)];
                                else
                                        index_ee_strong{i} = [index_ee_strong{i};
                                                find(effw_EE > thresh_EE & ...
                                                EE_seg.rowval <= corr_range & EE_colval >= (corr_seg - 1)*corr_range + 1 & ...
                                                EE_colval <= corr_seg*corr_range)];
                                end
                        end
...
save('strong_weights_turnover/turnover_corr__subdiag_rate.mat', 'ratio_n0_n1', 'n0_ee', 'n1_ee', 'n0_ie', 'n1_ie', 'resolution')
