...
save('trace_2/trace_2.mat', 'avg_EE_corr_diag', 'avg_EE_corr_subdiag', 'avg_EE_uncorr', ...
...
plot((0:(ckpt_num_all-1))*w_interval, avg_EE_corr_subdiag_connected, 'r', 'LineWidth', 2)
plot((0:(ckpt_num_all-1))*w_interval, avg_EE_uncorr_connected, 'k', 'LineWidth', 2)
plot([1 1]*8e3, [0, 0.06], 'k', 'LineWidth', 2, 'LineStyle', '--')
set(gcf, 'color', [1 1 1])   
ax = gca;
a = get(ax,'Label');
...
xlim([0 1.6e4])
legend('Diagonal', 'Whole range', 'Subdiagonal', 'Whole range', 'Uncorrelated', 'Whole range', 'Location', 'northwest')
savefig(gcf, 'trace_2/avg_wth_ratio_across_time.fig')
...
for ckpt = 1:ckpt_num_all
        if mod(ckpt, 20) == 1
                ...
                savefig(gcf, ['trace_2/avg_wth_ratio_', num2str(w_interval*(ckpt-1)), 's']);
        end
end
                
save('trace_2/trace_2_plot.mat', 'ckpt_num_all', 'w_interval', 'avg_EE_corr_diag_connected', 'avg_EE_corr_subdiag_connected', ...
