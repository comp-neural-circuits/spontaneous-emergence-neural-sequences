...
folder_name = splitdir(pwd())[end]
if isdigit(folder_name[2])
    trial_idx = parse(Int, folder_name[1:2])
    trial_digits = 2
else
    trial_idx = parse(Int, folder_name[1])
    trial_digits = 1
end
if isdigit(folder_name[end-2])
    ckpt_idx = parse(Int, folder_name[end-2:end])
    ckpt_digits = 3
else
    ckpt_idx = parse(Int, folder_name[end-1:end])
    ckpt_digits = 2
end
inject_point = parse(Int, folder_name[trial_digits+2:end-ckpt_digits-1])

if ckpt_idx > 150
    network_path = string("../../../basic_network/3/", trial_idx, "/")
    ckpt = (ckpt_idx - 149)*100
else
    network_path = string("../../../basic_network/2/", trial_idx, "/")
    ckpt = ckpt_idx - 79
end
...
E.inject_point = ((1:Ne) .== inject_point)
