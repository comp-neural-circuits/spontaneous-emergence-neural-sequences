...
network_folder_1 = '../../basic_network/1/';
network_folder_2 = '../../basic_network/2/';
...
parfor net = 1:net_num
        ...
        for ckpt = 1:ckpt_num
                ...
                if ckpt <= ckpt_num_1
                        network_dir = [network_folder_1, num2str(net), '/'];
                        ckpt_idx = ckpt;
                else
                        network_dir = [network_folder_2, num2str(net), '/'];
                        ckpt_idx = ckpt - ckpt_num_1 +ckpt_offset_2 + 1;
                end
                
                disp(['Net ', num2str(net), ', checkpoint ', num2str(ckpt)]);
        
                if ckpt_idx == 1
                        EE_seg = h5read_sparsematrix([network_dir, '/init_condition_w.h5'], '/EE');
                else
                        EE_seg = h5read_sparsematrix([network_dir, '/all_w.h5'], [num2str(ckpt_idx-1), '/EE']);
                end
                EE = retrieve_sp(EE_seg);
                
                if ckpt_idx == 1
                        ET = h5read([network_dir, '/init_condition_w.h5'], '/ET');
                else
                        ET = h5read([network_dir, '/all_w.h5'], ['/', num2str(ckpt_idx-1), '/ET']);
                end
...
save('triplet_motifs_EE_id.mat', 'n', 'E_num', 'cdf_all', 'mu', 'sigma', 'p_value', 'p0', 'motif_id');
